...
impl SqPackData {
    ...
    fn read_blocks(
        &mut self,
        block_offsets: impl Iterator<Item = u64>,
    ...
    fn read_model_raw(
        ...
        let block_header = read_and_parse!(
            self.file,
            base_offset + FileHeader::SIZE as u64,
            ModelBlockHeader
        );
        ...
        let (_, block_sizes) =
            Self::parse_block_sizes(&block_size_data, total_block_count).unwrap();
        ...
        let block_offsets = block_raw_offsets.map(|x| {
            base_offset
                + file_header.header_length as u64
                + block_header.offsets[0] as u64
                + x as u64
        });
        Ok(SqPackRawFile {
    ...
    fn serialize_model_header(block_header: &ModelBlockHeader) -> Vec<u8> {
        ...
        result
            .write_u16::<LittleEndian>(block_header.number_of_meshes)
            .unwrap();
        result
            .write_u16::<LittleEndian>(block_header.number_of_materials)
            .unwrap();
