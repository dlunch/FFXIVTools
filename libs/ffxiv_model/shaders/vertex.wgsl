...
fn vs_main(
	...
	[[location(1)]] bone_weight: vec4<i32>,
	[[location(2)]] bone_index: vec4<i32>,
	[[location(3)]] normal: vec4<f32>,
	[[location(4)]] tex_coord: vec4<f32>,
	[[location(5)]] bi_tangent: vec4<f32>,
	...
	for (var i: i32 = 0; i < 4; i = i + 1) {
		var index: i32 = bone_index[i];
		var weight: f32 = f32(bone_weight[i]) / 255.0;
		var boneTransform: mat4x4<f32> = mat4x4<f32>(
			bone_transform.transforms[index * 3],
			bone_transform.transforms[index * 3 + 1],
			bone_transform.transforms[index * 3 + 2],
	...
	var normalized_bi_tangent: vec4<f32> = normalize((bi_tangent * 2.0 / 255.0) - 1.0);
    ...
    out.tbn = mat4x4<f32>(
		vec4<f32>(tangent.x, normalized_bi_tangent.x, skinned_normal.x, 0.0),
		vec4<f32>(tangent.y, normalized_bi_tangent.y, skinned_normal.y, 0.0),
		vec4<f32>(tangent.z, normalized_bi_tangent.z, skinned_normal.z, 0.0),
		vec4<f32>(0.0, 0.0, 0.0, 1.0)
	);
