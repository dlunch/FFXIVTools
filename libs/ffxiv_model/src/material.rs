...
pub async fn create_material(renderer: &Renderer, context: &Context, mtrl: &Mtrl, textures: &[Arc<Texture>]) -> Material {
    ...
    match mtrl.shader_name() {
        ...
        "hair.shpk" => hair_material::HairMaterial::create(renderer, context, &mut textures),
        "iris.shpk" => iris_material::IrisMaterial::create(renderer, context, &mut textures),
        "skin.shpk" => skin_material::SkinMaterial::create(renderer, context, &mut textures),
        _ => panic!(),
    }
}
